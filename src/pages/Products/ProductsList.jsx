...
export default function ProductsList() {
    ...
    const search = useLocation().search;  //This tells us what the query(q) is.
    const searchTerm = new URLSearchParams(search).get("q");   // here accessing the value of query(q).
    
    useEffect(() => {
        async function fetchProducts() {
            try {
                await getProductList(searchTerm, initialProductList);
            } catch (err) {
