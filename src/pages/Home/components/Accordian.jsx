    ...
    return (
        <div>
            <h2 id={`accordion-flush-heading-${faq.id}`}>
                <button onClick={changeShow} type="button" className="text-lg flex items-center justify-between w-full py-5 font-medium text-left text-gray-500 border-b border-gray-200 dark:border-gray-700 dark:text-gray-400" data-accordion-target={`#accordion-flush-body-${faq.id}`} aria-expanded="true" aria-controls={`accordion-flush-body-${faq.id}`}>
            ...
            { show && <div id={`accordion-flush-body-${faq.id}`} className="" aria-labelledby={`accordion-flush-heading-${faq.id}`}>
